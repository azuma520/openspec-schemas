...
**The pairing unit is the subject, not the task.** A task MAY carry evidence for more than one subject. Records SHALL pair by their `subject:` value, and each subject appearing under a task SHALL have exactly one RED record and exactly one GREEN record. Subject values SHALL be unique within a task: two records claiming the same subject on the same side make the pairing indeterminate, which is the property these checks exist to guarantee. Ordinal pairing (the n-th RED with the n-th GREEN) SHALL NOT be used, because inserting one record silently re-pairs every record after it.
